...
func testExerciseType() workoutlog.ExerciseType {
	return workoutlog.ExerciseType{
		...
		VolumeConstraint: testVolumeConstratint,
	}
}
...
func TestHandleExercises(t *testing.T) {
	...
	Convey("Given an exercise manager", t, func() {
		...
		Convey("When we receive a request to create a new exercise type", func() {
			mockEmgr.On("NewExerciseType", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(&testExerciseID, nil)
		...
		Convey("When we receive a request to create a new exercise type of a non-unique name", func() {
			mockEmgr.On("NewExerciseType", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil, workoutlog.ErrNameNotUnique)
		...
		Convey("When we recieve a request for all exercise types", func() {
			mockEmgr.On("GetExerciseTypes", mock.Anything).Return([]workoutlog.ExerciseType{testExerciseType(), testExerciseType(), testExerciseType()}, nil)
			...
			for _, v := range types {
				So(v, ShouldResemble, testExerciseType())
			}
		})

		Convey("When we receive a request to update an exercise type", func() {
			mockEmgr.On("UpdateExerciseType", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil)
		...
		Convey("When updating an exercise type returns an error", func() {
			mockEmgr.On("UpdateExerciseType", mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(fmt.Errorf("an error"))
